...
function resolvePlan(plan: ProposedPlan, ctx: PlanContext): Candidate {
   ...
   */
  interface Draft { leg: PlanLeg; name: string; usd: string | "max" | { previous: number }; tokens: string | null; heldTokens: string | null }
  const drafts: Draft[] = [];
  for (const [index, leg] of plan.legs.entries()) {
    ...
    if (leg.op === "lend" && !def.earnSymbol) throw new Reject(name, `${leg.asset} has no Earn pool`);
    if (leg.op !== "lend" && !def.marginSymbol) throw new Reject(name, `${leg.asset} is not accepted by the margin account`);
    ...
    if (leg.op !== "lend" && !ctx.scope.smartAccount) throw new Reject(name, "a margin account is needed for this step and none is connected");
    if (leg.op !== "lend" && !ctx.capacity) throw new Reject(name, "the margin position was not read, so nothing touching the account can be sized");
    ...
    if (sizing.kind === "all_idle") {
      if (leg.op !== "lend" && leg.op !== "deposit_collateral") {
        throw new Reject(name, leg.op === "supply_blend"
          ...
          : "an idle wallet balance does not size a borrow or repay");
      }
      ...
      drafts.push({ leg, name, usd: held.usd, tokens: held.tokens, heldTokens: held.tokens });
      continue;
    }
    if (sizing.kind === "to_floor") {
      if (leg.op !== "borrow") throw new Reject(name, "only a borrow can be sized to the health-factor floor");
      drafts.push({ leg, name, usd: "max", tokens: null, heldTokens: null });
      continue;
    }
    if (sizing.kind === "previous_leg") {
      ...
      drafts.push({ leg, name, usd: { previous: index - 1 }, tokens: prev.tokens, heldTokens: null });
      continue;
    }
    ...
    drafts.push({ leg, name, usd, tokens, heldTokens: null });
  }
   ...
   */
  const marginDrafts = drafts.filter((d) => d.leg.op !== "lend" && d.leg.op !== "supply_blend");
  let sized: SizedLeg[] = [];
  ...
  if (marginDrafts.length) {
     ...
     */
    const canLowerHealth = marginDrafts.some((d) => d.leg.op === "borrow");
  ...
  const steps: ProposalStep[] = drafts.map((d, index) => {
    ...
    const symbol = d.leg.op === "lend" ? def.earnSymbol! : d.leg.op === "supply_blend" ? (def.marginSymbol ?? def.id) : wireSymbol(d.leg.asset);
    const verb = d.leg.op === "supply_blend" ? "Supply" : d.leg.op === "deposit_collateral" ? "Deposit" : d.leg.op.charAt(0).toUpperCase() + d.leg.op.slice(1);
    const where = d.leg.op === "lend" ? " to Earn" : d.leg.op === "supply_blend" ? " to Blend" : d.leg.op === "deposit_collateral" ? " as collateral" : "";
    const step: ProposalStep = {
      id: `s${index}-${d.leg.op}`,
      op: d.leg.op,
      asset: def.id,
      amount: d.tokens!,
      label: `${verb} ${d.tokens} ${def.displayLabel ?? def.id}${where}`,
      tool: TOOLS[d.leg.op],
      args: d.leg.op === "lend" ? { symbol, amount: d.tokens!, lender: ctx.scope.trader } : writeArgs(ctx.scope, symbol, d.tokens!),
