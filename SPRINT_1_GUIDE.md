...
- **D20 — foundation merged (2026-06-01).** PR #23 `s4/mercury-setup` (branch kept, not deleted): server-side **`/api/mercury` proxy** (JWT stays off the client; hardened from the original `NEXT_PUBLIC_` plan) + **`lib/mercury-client.ts`** (`mercuryQuery`). Verified: endpoint `https://testnet.mercurydata.app/graphql`, **standard `allContractEvents` indexing** (not a Retroshade), auth `Bearer JWT`, introspection works through the proxy, no JWT on the client. **Confirmed subscription-required-first** — `allContractEvents` errors until our contracts are subscribed. **Still to do for D20:** create the contract-event **subscription** (dashboard + reproducible `mercury-subscribe.mjs`) for AccountManager + 4 LendingPools, then D21 hooks.
- **Next:** finish D20 subscription → D21 event hooks → D22 analytics migration.
