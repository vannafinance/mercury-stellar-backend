...
- **D12 follow-up — complete (2026-06-01).** `PriceProvider` XLM source migrated off CoinGecko onto the on-chain Reflector oracle. Manual testnet check surfaced a regression: tick-driven `refresh()` called `fetchXlmPriceUsd(true)`, bypassing the 60s cache → CoinGecko hit every ~5s → HTTP 429. Fixed by sourcing XLM from `fetchTokenPrice("XLM")` (`lib/oracle-price.ts`); CoinGecko fetch + localStorage cache stripped from `lib/prices.ts` (kept `buildPrices`/`TokenPrices`/`getStablePrice`). `grep -rni coingecko` → empty (D30 criterion met early). +8 tests (`tests/contexts/price-context.test.tsx`, `tests/lib/prices.test.ts`). Dual `useTokenPrices` API still uncollapsed but now both read the same oracle (API-shape debt only). PR `s1/price-coingecko-to-oracle`.
...
- **D16–17 — complete (2026-05-30).** Unified mutation error/toast UX: `lib/errors/normalize.ts` centralises all Soroban error normalisation (6 exported functions replacing inline duplicates in 4 files); `hooks/use-mutation-toast.ts` declarative toast hook wired to earn supply/withdraw, portfolio deposit/withdraw, repay-loan, with loading→success→error flow; `hooks/use-wallet.ts` `useState({type:'',text:''})` anti-pattern removed, replaced with direct `toast.*` calls; `components/ui/message.tsx` dead code deleted. Build green, 80 tests passing. PR `s2/error-ux`.
- **D16–17 follow-up — complete (2026-06-01).** PR #17 only normalised earn/portfolio/wallet; manual testnet testing found Freighter-cancel (`XDR Read Error: …boundary of the buffer`) leaking raw across margin/farm/swap/lite. Fixed: added `isCancel` (now incl. `declined`) to `normalizeDepositCollateralError`/`normalizeTransferCollateralError`/`normalizeCreateAccountError`; routed `repay-loan-tab`, `leverage-assets-tab`, farm `add/remove-liquidity`, `SwapCard`, lite `one-click-strategy`+`position-detail`, `supply-liquidity-tab-new` through the normalizers. +27 cancel-detection tests (`tests/lib/errors-normalize.test.ts`); 115 passing. Audit item 11 (WB deposit+borrow → `useMutation`) still open — deferred to a follow-up. PR `s2/error-ux-cancel-normalize`.
- **D18–19 — complete (2026-05-30).** Optimistic updates for earn + margin: **Dev A** (`s3/optimistic-earn`) — `onMutate/onError/onSettled` added to `useSupplyLiquidity` and `useWithdrawLiquidity`; snapshots `['earn','userPositions']` cache, writes predicted deposited + vTokenBalance using current exchange rate, restores on error, reconciles with `onSettled: invalidateQueries(['earn'])`. **Dev B** (`s3/optimistic-margin`) — `onMutate/onError` added to `repayMutation` in `repay-loan-tab.tsx`; snapshots Zustand `borrowedBalances + avgHealthFactor`, predicts reduced debt and new HF, restores on rollback. Build green, 80 tests passing.
- **D20 — foundation merged (2026-06-01).** PR #23 `s4/mercury-setup` (branch kept, not deleted): server-side **`/api/mercury` proxy** (JWT stays off the client; hardened from the original `NEXT_PUBLIC_` plan) + **`lib/mercury-client.ts`** (`mercuryQuery`). Verified: endpoint `https://testnet.mercurydata.app/graphql`, **standard `allContractEvents` indexing** (not a Retroshade), auth `Bearer JWT`, introspection works through the proxy, no JWT on the client. **Correction (per Mercury team / Federico):** NO subscription needed — the old subscribe API is deprecated; Mercury indexes all contracts. Query via REST `GET /rest/events/by-ledger/contracts?contracts&from&to&limit` (the `allContractEvents` GraphQL table errors — don't use it).
- **D21 Dev A — merged (2026-06-05).** PR #24 `s4/mercury-events` (branch kept): `useMarginHistory` migrated to **Mercury Classic REST** — full history (no ~7-day cap), **dropped localStorage merge + ledger-tick refetch** (refreshes on mount/focus/margin-mutation). `app/api/mercury/events/route.ts` (REST proxy, server-side `account` filter on topic2 + `limit`, key server-side); `lib/mercury-client.ts` `fetchContractEvents()`+`decodeMercuryEvent()`; `lib/mercury-margin.ts` `getMarginHistoryFromMercury()` (one REST call, repays full, borrows `—` + carry-forward time). `useBlendEvents`/`useAquariusEvents` kept on RPC + TODO. **Flags:** contract gap (`Trader_Borrow` no amount/timestamp) → Rohit; per-account scale (`by-topics` vs Retroshade) → Mercury team.
- **Next:** D21 Dev B (`s4/mercury-events-soroswap-earn`): `useSoroswapEvents` + `useEarnTransactions` → Mercury; act on Federico's scale answer → D22 analytics migration.
