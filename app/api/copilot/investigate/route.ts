...
async function inputFrom(req: NextRequest): Promise<ResearchInput> {
  ...
  if (!isRecord(body) || Object.keys(body).some((key) => !["message", "wallet", "continuation"].includes(key)) ||
  ...
  return { message: body.message.trim(), wallet: body.wallet as string | null ?? null, continuation: body.continuation as string | null ?? null };
}
