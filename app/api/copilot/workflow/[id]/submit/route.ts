...
async function inputFrom(req: NextRequest): Promise<{ signedXdr: string }> {
  if (!req.body) throw new ResearchError("invalid_request", "Send the signed transaction envelope only.", 400);
  ...
  try {
    for (;;) {
      ...
      if (bytes > 110_000) { await reader.cancel(); throw new ResearchError("request_too_large", "This confirmation is too large.", 413); }
      chunks.push(value);
    }
  ...
  if (!isRecord(body) || Object.keys(body).some((key) => key !== "signedXdr") ||
    typeof body.signedXdr !== "string" || (body.signedXdr.length < 20 || body.signedXdr.length > 100_000)) {
    throw new ResearchError("invalid_request", "Send the signed transaction envelope only. This route cannot accept tools or amounts.", 400);
  }
  return { signedXdr: body.signedXdr };
}
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!/^[a-f0-9-]{36}$/.test(id)) {
    return NextResponse.json({ code: "invalid_request", message: "Send the signed transaction envelope only." }, { status: 400 });
  }
  let input: { signedXdr: string };
  ...
  try {
    const view = await withBoundUser(bound, () => submitWorkflow({
      id, signedXdr: input.signedXdr, subject: bound.sub, secret, server: copilotConfig.mcpBaseUrl, network,
