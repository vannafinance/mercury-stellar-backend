...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (req.body) {
    ...
    try {
      for (;;) {
        ...
        if (bytes > 4) { await reader.cancel(); return NextResponse.json({ code: "invalid_request", message: "This route cannot accept tools, amounts or envelopes." }, { status: 400 }); }
      }
  ...
  try {
    const view = await withBoundUser(bound, () => advanceWorkflow({
      id, subject: bound.sub, secret, server: copilotConfig.mcpBaseUrl, network,
    ...
    if (error instanceof WorkflowConflict) {
      return loaded.commit(NextResponse.json({ code: error.message, message: "This plan could not advance. Prepare a new one if it is stuck." }, { status: 409 }));
    }
