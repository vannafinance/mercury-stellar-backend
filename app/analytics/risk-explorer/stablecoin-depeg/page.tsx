...
import { formatUsd, cn } from "@/lib/analytics/utils";
...
// Mainnet: single Circle USDC shared across Blend / Aquarius / Soroswap.
type Stablecoin = "USDC";

const STABLECOIN_OPTIONS: { id: Stablecoin; label: string; color: string; desc: string }[] = [
  { id: "USDC", label: "Circle USDC", color: "#2775CA", desc: "Canonical USDC on Stellar mainnet — collateral & debt for all venues" },
];
...
const ALL_POSITIONS = Array.from({ length: 32 }, (_, i) => {
  ...
  const usdcPct = 0.25 + dr(i * 3) * 0.45;
  const trackPct = leverage >= 7 ? 0.3 + dr(i * 9) * 0.25 : 0.05 + dr(i * 9) * 0.2;
  const lpPct = Math.max(0, 1 - usdcPct - trackPct);
  return {
    id: i,
    debt,
...
function applyDepeg(pos: (typeof ALL_POSITIONS)[number], severity: number) {
  const depegFactor = 1 - severity / 100;
  const newUsdc = Math.floor(pos.usdcValue * depegFactor);
  const newMargin = newUsdc + pos.trackValue + pos.lpValue;
  ...
  return { ...pos, newMargin, newHF, liquidated, badDebt, newUsdc };
}
...
export default function StablecoinDepegPage() {
  ...
  const sim = useMemo(() => {
    ...
    const results = ALL_POSITIONS.map((p) => applyDepeg(p, effectiveSeverity));
    ...
    const totalExposure = ALL_POSITIONS.reduce((a, p) => a + p.usdcValue, 0);
    return {
      results,
      liquidated,
      totalBadDebt,
  ...
  const severityData = [2, 5, 10, 15, 20, 30, 50].map((sev) => {
    const res = ALL_POSITIONS.map((p) => applyDepeg(p, sev));
  ...
  return (
    <div className="p-6 w-full max-w-[1400px] mx-auto space-y-6">
      ...
      <PageHeader
        title="Stablecoin Depeg Scenario"
        subtitle="Model Circle USDC losing its peg on Stellar mainnet — collateral reprices and may breach Risk Engine HF≥1.1"
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              <div className="space-y-1.5">
                {STABLECOIN_OPTIONS.map((s) => (
                  <button
                    ...
                    className={cn(
                      "w-full flex items-center gap-2.5 p-2.5 rounded-r2 border text-left transition-all",
                      stablecoin === s.id ? "border-violet-300 bg-violet-50" : "border-vgray-100 hover:border-vgray-200",
                    )}
                  >
                    <CoinIcon symbol="USDC" size={18} />
                    <div>
              ...
              <div className="space-y-1.5">
                {RECOVERY_OPTIONS.map((r) => (
                  <button
                    ...
                    className={cn(
                      "w-full flex items-center justify-between p-2 rounded-r2 border text-left transition-all",
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-surface rounded-r4 border border-vgray-100 shadow-vanna p-4">
            ...
            <p className="text-[9px] text-vgray-400 mb-3">Positions liquidated at each severity level — find the critical depeg threshold</p>
            <ResponsiveContainer width="100%" height={160}>
              <BarChart data={severityData} barSize={28}>
                ...
                <Tooltip
                  contentStyle={cc.tooltip}
                  formatter={(v, name) => [name === "positions" ? `${v} positions` : `$${v}K bad debt`, String(name)]}
                />
                <Bar dataKey="positions" name="positions" radius={[3, 3, 0, 0]}>
                  {severityData.map((d, i) => (
                    <Cell key={i} fill={d.positions > 10 ? "#FC5457" : d.positions > 4 ? "#FF007A" : "#F59E0B"} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
