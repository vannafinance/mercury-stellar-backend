...
describe("investigation card / options", () => {
  it("submits a trimmed inline answer for a needs-input question", () => {
    const onAnswerQuestion = vi.fn();
    render(
      <InvestigationCard
        prompt="deposit XLM and borrow USDC"
        result={view({ status: "needs_input", question: "How much XLM should I deposit?" })}
        progress={null}
        loading={false}
        error={null}
    ...
    expect(onAnswerQuestion).toHaveBeenCalledWith("25");
    expect((input as HTMLInputElement).value).toBe("");
  });

  it("rejects an empty inline answer even if the form is submitted directly", () => {
    const onAnswerQuestion = vi.fn();
    render(
      <InvestigationCard
        prompt="deposit XLM and borrow USDC"
  ...
  it("disables the inline answer control while the investigation is busy", () => {
    render(
      <InvestigationCard
        prompt="deposit XLM and borrow USDC"
        result={view({ status: "needs_input", question: "How much XLM should I deposit?" })}
    ...
    expect((screen.getByRole("textbox", { name: "Your answer" }) as HTMLInputElement).disabled).toBe(true);
    expect((screen.getByRole("button", { name: "Send answer" }) as HTMLButtonElement).disabled).toBe(true);
  });

  it("keeps generic question copy and renders the inline answer form", () => {
    render(
      <InvestigationCard
        prompt="deposit XLM and borrow USDC"
        result={view({ status: "needs_input", question: "Which USDC market do you mean?" })}
        progress={null}
        loading={false}
        error={null}
        onAnswerQuestion={vi.fn()}
    ...
    expect(screen.getByText("Which USDC market do you mean?")).toBeTruthy();
    expect(screen.getByText("Reply below to continue.")).toBeTruthy();
