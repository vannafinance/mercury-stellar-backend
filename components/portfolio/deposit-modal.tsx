...
const ASSET_CONFIG: Record<string, { label: string; sub: string; bg: string; iconColor: string }> = {
  XLM:           { label: "XLM",     sub: "Stellar Lumens",  bg: "#703AE6", iconColor: "#fff" },
  USDC:          { label: "USDC",    sub: "USD Coin",        bg: "#2775CA", iconColor: "#fff" },
  AQUARIUS_USDC: { label: "AQU",     sub: "Aquarius USDC",   bg: "#00B2FF", iconColor: "#fff" },
  SOROSWAP_USDC: { label: "SRS",     sub: "Soroswap USDC",   bg: "#9333EA", iconColor: "#fff" },
};

const AssetIcon = ({ asset, size = 36 }: { asset: string; size?: number }) => {
  const cfg = ASSET_CONFIG[asset] ?? { bg: "#703AE6", iconColor: "#fff", label: asset[0] };
  return (
    <div
      className="rounded-full flex items-center justify-center font-bold shrink-0"
      style={{ width: size, height: size, background: cfg.bg, color: cfg.iconColor, fontSize: size * 0.36 }}
    >
      {cfg.label.slice(0, 2)}
    </div>
  );
          ...
          >
            ...
            <div className="px-6 py-5 flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-2">
                ...
                {/* Balance + quick amounts — only for XLM */}
                {selectedAsset === ASSET_TYPES.XLM && (
                  <div className="flex flex-col gap-2">
                    <div className={`flex items-center justify-between text-[12px] ${isDark ? "text-[#777]" : "text-[#9ca3af]"}`}>
                      <span>Available balance</span>
                      <span className={`font-semibold ${isDark ? "text-[#A0A0A0]" : "text-[#374151]"}`}>
                        {balance} XLM
                      </span>
                    </div>
                    <div className="grid grid-cols-4 gap-2">
            ...
            <div className={`px-6 pb-5 flex gap-3`}>
              <button
                ...
                onClick={handleClose}
                disabled={isLoading}
                className={`flex-1 h-11 rounded-xl text-[14px] font-semibold transition-colors cursor-pointer ${
                  isDark
