...
| 2 | "when XLM hits $0.50 sell everything" still falls to the generic capabilities blurb — not fixed, and out of scope for a quick fix. | Low, needs product decision | `lib/copilot/router.ts` | There is no `sell` / liquidate-everything op at all, so no amount of trigger-word tuning routes this to a write for the guard below to catch. Needs a real op (and its own risk/confirmation path) before a conditional wrapped around it means anything. |
...
| 2 | `note.txt` §18's expected test count (778) is stale. | Update to 821. |
